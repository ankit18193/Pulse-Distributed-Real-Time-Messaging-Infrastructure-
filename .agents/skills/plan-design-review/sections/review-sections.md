...
**STOP.** ask_question once per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** ask_question once per issue. Do NOT batch. Recommend + WHY.
...
Each decision = one ask_question with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.
...
ask_question: "The review passes changed [list major design changes]. Want me to regenerate mockups to reflect the updated plan? This ensures the visual reference matches what we're actually building."
...
Follow the ask_question format from the Preamble above. Additional rules for plan design reviews:
...
* **Zero findings:** if a section has zero findings, state "No issues, moving on" and proceed. Otherwise, use ask_question for each gap — a gap with an "obvious fix" is still a gap and still needs user approval before any change lands in the plan.
* **NEVER use ask_question to ask which variant the user prefers.** Always create a comparison board first (`$D compare --serve`) and open it in the browser. The board has rating controls, comments, remix/regenerate buttons, and structured feedback output. Use ask_question ONLY to notify the user the board is open and wait for them to finish — not to present variants inline and ask "which do you prefer?" That is a degraded experience.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual ask_question. Never batch TODOs — one per question. Never silently skip this step.
...
If any ask_question goes unanswered, note it here. Never silently default to an option.
...
```bash
~/.agents/skills/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
  ~/.agents/skills/bin/gstack-brain-cache invalidate brand --project "$SLUG" 2>/dev/null || true
```
