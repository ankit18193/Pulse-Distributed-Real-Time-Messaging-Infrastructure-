...
description: >-
  Designer's eye plan review: Rate each design dimension 0-10, explain what a 10 looks like, and iterate on UI/UX, states, and visual design. Use when asked to "review design plan", "design critique", or "review ux plan".
---
...
1. First tool call = ask_question (tool_use). Confirm what to review.
2. Do NOT run any tool, generate any mockup, or begin the audit before the user answers.
3. If ask_question is disallowed (`--disallowedTools`), render the options as plain prose — each on its own line starting with the letter and paren at column 0 (no blockquote, no leading `>`) — then STOP and wait. Use exactly this shape:
...
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "brand"
  ~/.agents/skills/bin/gstack-brain-cache get brand --project "$SLUG" 2>/dev/null || printf '_(no brand digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
...
### 0D. Focus Areas
ask_question: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. I'll generate visual mockups next, then review all 7 dimensions. Want me to focus on specific areas instead of all 7?"
...
path; use this for the ask_question URL AND as the base for the reload
...
**PRIMARY WAIT: ask_question with board URL**

After the board is serving, use ask_question to wait for the user. Include the
...
**Do NOT use ask_question to ask which variant the user prefers.** The comparison
board IS the chooser. ask_question is just the blocking wait mechanism.

**After the user responds to ask_question:**
...
5. Reload the board in the user's browser (same tab) — the URL is per-board
   ...
   daemon.
6. The board auto-refreshes. **ask_question again** with the same board URL to
   wait for the next round of feedback. Repeat until `feedback.json` appears.

**If `NO_FEEDBACK_FILE`:** The user typed their preferences directly in the
ask_question response instead of using the board. Use their text response
as the feedback.
...
Use ask_question to verify before proceeding.
...
**Do NOT use ask_question to ask which variant the user picked.** Read `feedback.json` — it already contains their preferred variant, ratings, comments, and overall feedback. Only use ask_question to confirm you understood the feedback correctly, never to re-ask what they chose.
...
**Log the result:**
```bash
~/.agents/skills/bin/gstack-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
5. ask_question if there's a genuine design choice to resolve
...
> **STOP.** Before running the 7 design passes, required outputs, and review report (only after Step 0 scope is agreed), Read `~/.agents/skills/plan-design-review/sections/review-sections.md` and execute it
