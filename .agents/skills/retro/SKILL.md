...
name: retro
description: >-
  Weekly engineering retrospective: Analyzes git commit patterns, shipping cadence, context switching, and team highlights with streaks and improvement areas. Use when asked to "weekly retro", "retrospective", or "what did we ship".
---
...
**Midnight-aligned windows:** For day (`d`) and week (`w`) units, compute an absolute start date at local midnight, not a relative string. For example, if today is 2026-03-18 and the window is 7 days: the start date is 2026-03-11. Use `--since "2026-03-11T00:00:00"` — the explicit `T00:00:00` suffix ensures git starts from midnight. Without it, git uses the current wall-clock time (e.g., `--since "2026-03-11"` at 11pm means 11pm, not midnight). For week units, multiply by 7 to get days (e.g., `2w` = 14 days back). For hour (`h`) units, use `--since "N hours ago"` since midnight alignment does not apply to sub-day windows. Compute "today" from the user-visible `## currentDate` tag in the session reminder — NEVER from `date` (the system clock can be hours off in containerized harnesses). If you cannot reliably compute "today", stop and ask the user via ask_question rather than proceeding.
...
```bash
_RM="$HOME/.agents/skills/bin/gstack-retro-metrics"
[ -x "$_RM" ] || _RM=".agents/skills/bin/gstack-retro-metrics"
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
accepted a Completeness ≤ 7 option (see the ask_question Format section). Zero
...
> **STOP.** Before writing the retrospective narrative (Step 14, after all metrics are computed and compared), Read `~/.agents/skills/retro/sections/report-format.md` and execute it
...
```bash
DISCOVER_BIN=""
[ -x ~/.agents/skills/bin/gstack-global-discover ] && DISCOVER_BIN=~/.agents/skills/bin/gstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x .agents/skills/bin/gstack-global-discover ] && DISCOVER_BIN=.agents/skills/bin/gstack-global-discover
