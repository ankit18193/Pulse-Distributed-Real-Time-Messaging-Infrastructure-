...
name: cso
description: >-
  Chief Security Officer audit: Security inspection covering secret archaeology, dependency supply chain, OWASP Top 10, STRIDE threat modeling, and access control. Use when asked to "security audit", "check vulnerabilities", or "cso review".
---
...
> **STOP.** Before running the scope-dependent audit phases (Phases 2-11) selected by the resolved mode, after the Phase 0 stack detection and Phase 1 attack-surface census, Read `~/.agents/skills/cso/sections/audit-phases.md` and execute it
...
**Remediation Roadmap:** For the top 5 findings, present via ask_question:
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
