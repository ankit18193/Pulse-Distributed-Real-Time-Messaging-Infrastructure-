...
name: investigate
description: >-
  Systematic root-cause debugging: 4-phase protocol (Reproduction & Evidence → Instrumentation & Tracing → Root Cause Identification → Fix & Regression Verification). Use when asked to "debug this", "investigate error", "fix this bug", or "why is this broken".
---
...
1. **Collect symptoms:** Read the error messages, stack traces, and reproduction steps. If the user hasn't provided enough context, ask ONE question at a time via ask_question.
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.agents/skills/bin/gstack-learnings-search --limit 10 --query "debug investigation root cause hypothesis bug fix" --cross-project 2>/dev/null || true
else
  ~/.agents/skills/bin/gstack-learnings-search --limit 10 --query "debug investigation root cause hypothesis bug fix" 2>/dev/null || true
fi
```
...
_FREEZE_SCRIPT="$HOME/.agents/skills/freeze/bin/check-freeze.sh"
...
3. **3-strike rule:** If 3 hypotheses fail, **STOP**. Use ask_question:
   ```
...
5. **If the fix touches >5 files:** Use ask_question to flag the blast radius:
   ```
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"investigate","type":"investigation","key":"ROOT_CAUSE_KEY","insight":"ROOT_CAUSE_SUMMARY","confidence":9,"source":"observed","files":["affected/file1.ts","affected/file2.ts"]}'
```
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"investigate","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- **If fix touches >5 files → ask_question** about blast radius before proceeding.
- **Completion status:**
