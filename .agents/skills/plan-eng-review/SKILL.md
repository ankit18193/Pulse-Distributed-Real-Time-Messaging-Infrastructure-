...
description: >-
  Eng manager-mode plan review: Lock in execution plan, architecture, data flow, ASCII diagrams, edge cases, failure modes, and test coverage before writing code. Use when asked to "review the architecture", "engineering review", "check implementation plan", or "lock in the plan".
---
...
If the complexity check triggers (8+ files or 2+ new classes/services), STOP before any review-section work. Call ask_question: name what's overbuilt, propose a minimal version that achieves the core goal, ask whether to reduce or proceed as-is. The ask_question call is a tool_use, not prose — call the tool directly.

**STOP.** Do NOT proceed to Section 1 (Architecture review), edit the plan file with a proposed scope reduction, or call Antigravity Plan Approval until the user responds. Naming the 80% solution in chat prose and continuing — or loading the ask_question schema via ToolSearch and then never invoking it — is the failure mode this gate exists to prevent.
...
> **STOP.** Before running the 4-section review, outside voice, required outputs, and review report (only after Step 0 scope is agreed), Read `~/.agents/skills/plan-eng-review/sections/review-sections.md` and execute it
