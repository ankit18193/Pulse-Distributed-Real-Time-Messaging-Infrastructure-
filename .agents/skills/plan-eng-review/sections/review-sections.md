...
**Anti-shortcut clause:** The plan file is the OUTPUT of the interactive review, not a substitute for it. Writing every finding into one plan write and calling Antigravity Plan Approval without firing ask_question is the precise failure mode of the May 2026 transcript bug — the model explored, found issues, and dumped them into a deliverable rather than walking the user through them. If you have ANY non-trivial finding in any review section, the path from finding to Antigravity Plan Approval goes THROUGH ask_question. Zero findings in every section is the only path to Antigravity Plan Approval that bypasses ask_question. If you find yourself wanting to write a plan with findings before asking, stop and call ask_question now — that's the bug, recognize it.
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.agents/skills/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.agents/skills/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
For each issue found in this section, call ask_question individually. One issue per call. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one ask_question. Use the preamble's ask_question Format section. The ask_question call is a tool_use, not prose — call the tool directly.

**STOP.** Do NOT proceed to the next review section, edit the plan file with the proposed fix, or call Antigravity Plan Approval until the user responds. An issue with an "obvious fix" is still an issue and still needs explicit user approval before it lands in the plan. Loading the ask_question schema via ToolSearch and then writing the recommendation as chat prose is the failure mode this gate exists to prevent.
...
**IRON RULE:** When the coverage audit identifies a REGRESSION — code that previously worked but the diff broke — a regression test is added to the plan as a critical requirement. No ask_question. No skipping. Regressions are the highest-priority test because they prove something broke.
...
For LLM/prompt changes: check the "Prompt/LLM changes" file patterns listed in AGENTS.md. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against. Then use ask_question to confirm the eval scope with the user.
...
_TEL=$(~/.agents/skills/bin/gstack-config get telemetry 2>/dev/null || echo off)
_CODEX_CFG=$(~/.agents/skills/bin/gstack-config get codex_reviews 2>/dev/null || echo enabled)
source ~/.agents/skills/bin/gstack-codex-probe 2>/dev/null || true
...
For each substantive tension point, use ask_question:
...
**Persist the result:**
```bash
~/.agents/skills/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
finding via ask_question and getting explicit approval. This applies even when you
...
Follow the ask_question format from the Preamble above. Additional rules for plan reviews:
* **One issue = one ask_question call.** Never combine multiple issues into one question.
...
* **Coverage vs kind:** for every per-issue ask_question you raise in this review, decide whether the options differ in coverage or in kind. If coverage (e.g., more tests vs fewer, complete error handling vs happy-path-only, full edge-case coverage vs shortcut), include `Completeness: N/10` on each option. If kind (e.g., architectural choice between two different systems, posture-over-posture, A/B/C where each is a different kind of thing), skip the score and add one line: `Note: options differ in kind, not coverage — no completeness score.` Do NOT fabricate scores on kind-differentiated questions — filler scores are worse than no score.
* **Zero findings:** if a section has zero findings, state "No issues, moving on" and proceed. Otherwise, use ask_question for each finding — a finding with an "obvious fix" is still a finding and still needs user approval before any change lands in the plan.
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual ask_question. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `~/.agents/skills/review/TODOS-format.md`.
...
```bash
~/.agents/skills/bin/gstack-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
~/.agents/skills/bin/gstack-decision-log '{"decision":"Eng review (MODE): ARCH_SUMMARY","rationale":"KEY_DECISION","scope":"branch","source":"skill","confidence":8}' 2>/dev/null || true
```
...
```bash
~/.agents/skills/bin/gstack-review-read
```
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"plan-eng-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
1. Brain trust policy for the active endpoint is `personal` (check via
   `~/.agents/skills/bin/gstack-config get brain_trust_policy@<endpoint-hash>`).
...
```bash
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
...
```bash
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
(~/.agents/skills/bin/gstack-brain-cache refresh --project "$SLUG" 2>/dev/null &) || true
```
...
Use ask_question with only the applicable options:
- **A)** Run /plan-design-review (only if UI scope detected and no design review exists)
- **B)** Run /plan-ceo-review (only if significant product change and no CEO review exists)
- **C)** Ready to implement — run /ship when done

## Unresolved decisions
If the user does not respond to an ask_question or interrupts to move on, note which decisions were left unresolved. At the end of the review, list these as "Unresolved decisions that may bite you later" — never silently default to an option.
