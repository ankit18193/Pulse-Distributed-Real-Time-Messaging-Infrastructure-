...
name: plan-tune
description: >-
  Interactive question sensitivity tuner: Tune AskUserQuestion / decision brief thresholds per category and configure developer preferences. Use when asked to "tune questions", "question preferences", or "/plan-tune".
---

## When to invoke this skill

Review which ask_question prompts fire across gstack skills, set per-question preferences
...
10. **"Turn it off" / "disable"** → `~/.agents/skills/bin/gstack-config set question_tuning false`
11. **"Turn it on" / "enable"** → `~/.agents/skills/bin/gstack-config set question_tuning true && touch ~/.gstack/.question-tuning-prompted`
...
1. Detect contributor state (for prompt framing only, not for auto-action):
   ...
   _CONTRIB=$(~/.agents/skills/bin/gstack-config get gstack_contributor 2>/dev/null || echo "false")
   echo "QUESTION_TUNING: $_QT"
   echo "CONTRIBUTOR: $_CONTRIB"
   ```

2. ask_question (use the contributor-specific framing only if `_CONTRIB=true`,
   ...
   **Contributor framing (only if `_CONTRIB=true`):**
   ...
   > ask_question outcome locally to
...
4. If A or B: enable:
   ```bash
   ~/.agents/skills/bin/gstack-config set question_tuning true
   ```
...
1. Ask FIVE one-per-dimension declaration questions via individual
   ask_question calls (one at a time). Use plain English, no jargon:
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   ~/.agents/skills/bin/gstack-developer-profile --read >/dev/null
...
```bash
~/.agents/skills/bin/gstack-developer-profile --profile
```
...
4. Write:
   ```bash
   ~/.agents/skills/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.agents/skills/bin/gstack-developer-profile --gap
```
...
```bash
~/.agents/skills/bin/gstack-question-preference --stats
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)"
eval "$(~/.agents/skills/bin/gstack-paths)"
_LOG="$GSTACK_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ -f "$_LOG" ]; then
...
~/.agents/skills/bin/gstack-developer-profile --profile | bun -e "
...
"
echo '---DISTILL---'
~/.agents/skills/bin/gstack-distill-free-text --status
```
...
1. Show the proposals:
   ```bash
   ~/.agents/skills/bin/gstack-distill-apply --list
   ```

2. For each unapplied proposal, present it as a numbered item and use
   ask_question (one per call, per skill convention). Show:
...
3. **On accept** (Y): apply via the bin. The skill also publishes the
   ...
   ~/.agents/skills/bin/gstack-distill-apply --proposal N --gbrain-published true|false
   ```

   For `preference`:
   ```bash
   ~/.agents/skills/bin/gstack-distill-apply --proposal N
   ```
...
1. Run distill:
   ```bash
   ~/.agents/skills/bin/gstack-distill-free-text
   ```
...
3. If `NO_FREE_TEXT`: tell the user "No free-text answers since the last
   distill. Keep using gstack — `Other` responses on ask_question feed
   this loop."
...
```bash
~/.agents/skills/bin/gstack-distill-free-text --background
```
