...
name: browse
description: >-
  Fast browser automation & inspection: Navigates URLs, verifies DOM elements, inspects layouts, captures screenshots, and checks responsive views. Use when asked to "browse", "open in browser", "test page", or "take screenshot".
---
...
# 2. Tell the user what happened (via ask_question)
...
> **STOP.** Before using any command or snapshot flag beyond the Most-Used Commands table — the full generated reference for every browse command, its argument shape, and every snapshot flag, Read `~/.agents/skills/browse/sections/command-list.md` and execute it
