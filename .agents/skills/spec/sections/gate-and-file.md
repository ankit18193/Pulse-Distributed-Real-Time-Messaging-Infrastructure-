...
ask_question: A) edit, B) acknowledge and proceed, C) cancel. **On a PUBLIC repo,
...
bun ~/.agents/skills/lib/redact-audit-log.ts \
...
block; MEDIUM PII/legal/internal confirm via ask_question; LOW surfaces). Full
...
REDACT_BODY_EOF
REDACT_JSON=$(~/.agents/skills/bin/gstack-redact --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json)
REDACT_CODE=$?
```
...
2. **Exit 2 (MEDIUM)** — ask_question per finding (cluster identical ids; PUBLIC
...
- **Score <7, iteration 2:** print "Quality gate: {score}/10 (after one
  revision). Codex still flags: {ambiguities}." ask_question:
  - A) Ship anyway (file at this quality)
  - B) Save draft locally and stop (no issue filed)
  - C) One more revision attempt

Max 3 dispatches total. If still <7 after iter 3, ask_question same options.
...
reuse it; write the exact bytes to `$REDACT_FILE`; `~/.agents/skills/bin/gstack-redact --from-file "$REDACT_FILE"
...
~/.agents/skills/bin/gstack-decision-log '{"decision":"Spec filed #ISSUE_NUMBER: TITLE","rationale":"APPROACH","scope":"issue","issue":"ISSUE_NUMBER","source":"skill","confidence":7}' 2>/dev/null || true
```
...
If `$DIRTY` is non-empty, ask_question:
...
from the answer, re-prompt the ask_question. The check must happen INSIDE
...
**D16 mandatory final-confirm gate:** ask_question: "Spawn agent now? Last
...
- `T_PHASE1_START` — Phase 1 first ask_question or first text emit
