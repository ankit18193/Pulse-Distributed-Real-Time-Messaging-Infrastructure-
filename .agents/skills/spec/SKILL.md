...
name: spec
description: >-
  Executable specification author: Transforms vague intent into a precise, phased 5-part engineering specification with issue filing and task planning. Use when asked to "write a spec", "create feature spec", or "spec out".
---
...
gh issue list --search "<keywords>" --state open --limit 10 --json number,title,url 2>/dev/null \
  | jq -r '.[] | "#\(.number) \(.title)"' \
  | ~/.agents/skills/bin/gstack-issue-guard --stdin --source issue-dedupe 2>/dev/null || true
```
...
- **1+ matches:** surface them to the user via ask_question: "Found {N} similar
...
> **STOP.** Before running the quality gate and filing the spec (Phases 4.5-5, once the user confirms the Phase 4 draft), Read `~/.agents/skills/spec/sections/gate-and-file.md` and execute it
...
`ask_question`. For open-ended interrogation, ask inline in the chat — the
