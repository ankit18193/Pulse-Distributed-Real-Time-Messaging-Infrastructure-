...
name: qa-only
description: >-
  Report-only QA testing: Audits web application via browser testing and produces a comprehensive bug and readiness report without modifying source code. Use when asked to "qa-only", "qa report", or "audit without fixes".
---
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"qa-only","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
