...
```bash
source <(~/.agents/skills/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.agents/skills/bin/gstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.agents/skills/review/specialists/testing.md`
2. **Maintainability** — read `~/.agents/skills/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.agents/skills/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.agents/skills/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.agents/skills/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.agents/skills/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.agents/skills/review/design-checklist.md`
8. **Simplification** — if DIFF_LINES > 100. Read `~/.agents/skills/review/specialists/simplification.md`. Advisory-only lens: hunts unrequested structure (hand-rolled stdlib, one-implementation abstractions, dependencies duplicating platform features), never coverage.
...
```bash
~/.agents/skills/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.agents/skills/review/specialists/red-team.md`
