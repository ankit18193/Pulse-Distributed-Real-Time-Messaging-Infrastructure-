...
name: ship
description: >-
  Fully automated release & ship workflow: Verifies working tree, runs test suites, performs pre-landing review, bumps version, generates changelog, creates bisectable commits, pushes, and opens PR. Use when asked to "ship it", "create a pr", or "deploy this".
---
...
**STOP and Read `~/.agents/skills/ship/sections/apple-release.md` FIRST**
...
For Design Review: run `source <(~/.agents/skills/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
3. **If no release pipeline exists and a new artifact was added:** Use ask_question:
...
> **STOP.** Before running the test suites and (if prompt files changed) the eval suites (Steps 4-6), Read `~/.agents/skills/ship/sections/tests.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing test coverage of the diff (Step 7), Read `~/.agents/skills/ship/sections/test-coverage.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before auditing plan completion, verification, and scope drift (Step 8), Read `~/.agents/skills/ship/sections/plan-completion.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the pre-landing review and specialist dispatch (Step 9), Read `~/.agents/skills/ship/sections/review-army.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before addressing Greptile review comments when a PR exists (Step 10), Read `~/.agents/skills/ship/sections/greptile.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

> **STOP.** Before the adversarial review and learnings capture (Step 11), Read `~/.agents/skills/ship/sections/adversarial.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
1. **Classify state** — pure reader, never writes:
   ```bash
   bun run ~/.agents/skills/bin/gstack-version-bump classify --base <base>
   ```
   Read the JSON `state` and dispatch:
   - **FRESH** → do the bump (steps 2-4).
   - **ALREADY_BUMPED** → skip the bump, but run the queue-drift check (step 3) with the reported `currentVersion`. If the queue moved (next free version differs), **ask_question**: rebump to the new version (rewrites CHANGELOG header + PR title) or keep current (CI version-gate will reject until resolved).
...
3. **Queue-aware pick** (workspace-aware ship):
   ```bash
   QUEUE_JSON=$(bun run ~/.agents/skills/bin/gstack-next-version --base <base> --bump "$BUMP_LEVEL" --current-version "$BASE_VERSION" 2>/dev/null || echo '{"offline":true}')
   NEW_VERSION=$(echo "$QUEUE_JSON" | jq -r '.version // empty')
   ```
   If `offline`/util fails: fall back to local `BUMP_LEVEL` arithmetic and print `⚠ workspace-aware ship offline — using local bump only`. If `claimed` is non-empty, render the queue table so the user sees landing order. If an active sibling workspace holds a version `>= NEW_VERSION`, **ask_question**: advance past (unrelated work) or abort and sync with the sibling.

4. **Write the bump** (FRESH, or an approved rebump):
   ```bash
   bun run ~/.agents/skills/bin/gstack-version-bump write --version "$NEW_VERSION" --regen-digest
   ```
...
5. **Record the release decision** (durable cross-session memory). The bump level is a real decision the next session should not re-derive blind:
   ```bash
   ~/.agents/skills/bin/gstack-decision-log '{"decision":"Ship NEW_VERSION (BUMP_LEVEL)","rationale":"WHY","scope":"repo","source":"skill","confidence":9}' 2>/dev/null || true
   ```
...
> **STOP.** Before writing the CHANGELOG entry (Step 13), Read `~/.agents/skills/ship/sections/changelog.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
**If TODOS.md does not exist:** Use ask_question:
...
**If disorganized** (missing priority fields, no component groupings, no Completed section): Use ask_question:
...
user via ask_question rather than destroying non-WIP commits.
...
```bash
~/.agents/skills/bin/gstack-evidence check --label tests --expect-cmd '<exact tests-lane command from Step 5>' --label vitest --expect-cmd '<exact vitest-lane command from Step 5>' --max-age 24 --allow-paths CHANGELOG.md,VERSION,package.json,agents-digest/gstack-AGENTS.md
```
...
- **Any STALE/MISSING (exit non-zero):** run live, wrapped, so the fresh run is
  recorded: `~/.agents/skills/bin/gstack-evidence run --label <lane> -- '<command>'`.
...
```bash
_REDACT_PREPUSH=$(~/.agents/skills/bin/gstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   ~/.agents/skills/bin/gstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   offer (fires once EVER, machine-wide). ask_question:
   ...
   If A: run `~/.agents/skills/bin/gstack-config set redact_prepush_hook true`
   then `~/.agents/skills/bin/gstack-redact install-prepush-hook`.
   If B: run `~/.agents/skills/bin/gstack-config set redact_prepush_hook false`.
   ALWAYS (after either answer, but NOT if the question itself failed to
   render — a failed ask_question must re-offer next time):
   ```bash
...
> **STOP.** Before dispatching the /document-release subagent to sync docs (Step 18) and then creating or updating the PR/MR (Step 19), Read `~/.agents/skills/ship/sections/pr-body.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.agents/skills/bin/gstack-review-log '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"'"$(git rev-parse --abbrev-ref HEAD)"'"}'
```
...
_QT=$(~/.agents/skills/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
if [ ! -f "$_NUDGE_MARKER" ] && [ "$_QT" = "false" ]; then
  echo ""
  echo "gstack can learn from your ask_question answers. Run /plan-tune to opt in"
