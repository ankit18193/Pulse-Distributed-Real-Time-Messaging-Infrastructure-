...
```bash
~/.agents/skills/bin/gstack-evidence run --label tests -- 'bin/test-lane 2>&1' &
~/.agents/skills/bin/gstack-evidence run --label vitest -- 'npm run test 2>&1' &
wait
```
...
`~/.agents/skills/bin/gstack-detach` instead — it survives in its own
...
```bash
~/.agents/skills/bin/gstack-detach --label ship-evals --lock gstack-evals --timeout 5400 -- <project eval command>
```
