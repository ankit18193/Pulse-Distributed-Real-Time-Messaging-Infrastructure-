...
for PLAN_DIR in "$HOME/.gstack/projects/$_PLAN_SLUG" "$HOME.agents/plans" "$HOME/.codex/plans" ".gstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
1. **Any NOT DONE items** (highest priority — known missing work). Use ask_question:
...
2. **Any UNVERIFIABLE items** (silent gaps — the diff cannot prove them either way). Only fires after NOT DONE is resolved or absent.
   ...
   **Per-item confirmation is mandatory.** Do NOT use a single ask_question to blanket-confirm all UNVERIFIABLE items. Blanket confirmation is the failure mode that surfaced in VAS-449 (user clicks A without opening any file). Instead:

   - Loop through UNVERIFIABLE items one at a time.
   - For each item, use ask_question with the item's *specific* manual check (e.g., "Confirm: does `~/Development/domain-hq/docs/dashboard.md` exist?", not "Have you checked all items?").
   - Options per item:
...
3. If `deferred > 0` or `unverifiable > 0` and no user override, present the items via the appropriate ask_question (see Gate Logic priority order above) before continuing.
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.agents/skills/bin/gstack-learnings-search --limit 10 --query "release ship version changelog merge pr" --cross-project 2>/dev/null || true
else
  ~/.agents/skills/bin/gstack-learnings-search --limit 10 --query "release ship version changelog merge pr" 2>/dev/null || true
fi
```
