...
description: >-
  Developer experience (DX) plan review: Measure Time-to-Hello-World (TTHW), identify magical moments, eliminate friction points, and audit onboarding journeys. Use when asked to "devex review", "dx plan review", or "audit developer experience".
---
...
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "developer-persona"
  ~/.agents/skills/bin/gstack-brain-cache get developer-persona --project "$SLUG" 2>/dev/null || printf '_(no developer-persona digest available yet)_\n'
  printf '\n### %s\n\n' "recent-decisions"
  ...
  printf '\n### %s\n\n' "competitive-intel"
  ~/.agents/skills/bin/gstack-brain-cache get competitive-intel --project "$SLUG" 2>/dev/null || printf '_(no competitive-intel digest available yet)_\n'
...
Then SHOW it to the user via ask_question:
...
Load the "## Pass 1" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`
...
3. **ask_question per friction point.** One question per friction point found.
...
7. ask_question if there's a genuine DX choice to resolve
...
> **STOP.** Before running the 8 DX passes, required outputs, and review report (only after Step 0 investigation is complete), Read `~/.agents/skills/plan-devex-review/sections/review-sections.md` and execute it
