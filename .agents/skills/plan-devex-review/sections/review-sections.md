...
~/.agents/skills/bin/gstack-review-read 2>/dev/null | grep plan-devex-review || echo "NO_PRIOR_DX_REVIEWS"
```
...
Load reference: Read the "## Pass 1" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
**STOP.** ask_question once per issue. Recommend + WHY. Reference the persona.
...
Load reference: Read the "## Pass 2" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
**STOP.** ask_question once per issue. Recommend + WHY.
...
Load reference: Read the "## Pass 3" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 4" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 5" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 6" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 7" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
Load reference: Read the "## Pass 8" section from `~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.
...
`~/.agents/skills/plan-devex-review/dx-hall-of-fame.md`.

Check each item. For any unchecked item, explain what's missing and suggest the fix.

**STOP.** ask_question for any item that requires a design decision.
...
Follow the ask_question format from the Preamble above. Additional rules for
DX reviews:

* **One issue = one ask_question call.** Never combine multiple issues.
...
* **Zero findings:** if a section has zero findings, state "No issues, moving on"
  and proceed. Otherwise, use ask_question for each gap — a gap with an
...
ask_question. Never batch. For DX debt: missing error messages, unspecified upgrade
...
If any ask_question goes unanswered, note here. Never silently default.
...
```bash
~/.agents/skills/bin/gstack-review-log '{"skill":"plan-devex-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"product_type":"PRODUCT_TYPE","tthw_current":"TTHW_CURRENT","tthw_target":"TTHW_TARGET","mode":"MODE","persona":"PERSONA","competitive_tier":"COMPETITIVE_TIER","unresolved":N,"commit":"COMMIT"}'
```
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
  ~/.agents/skills/bin/gstack-brain-cache invalidate developer-persona --project "$SLUG" 2>/dev/null || true
```
...
Use ask_question with applicable options:
