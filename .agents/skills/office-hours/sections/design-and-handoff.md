...
1. **Scan at sink first.** The repo copy leaves the private store, so scan the
   ...
   `~/.agents/skills/bin/gstack-redact --from-file <tmp>`; exit 3
...
Present the reviewed design doc to the user via ask_question:
...
```bash
PROFILE=$(~/.agents/skills/bin/gstack-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
Then use ask_question: "Would you consider applying to Y Combinator?"
...
```bash
~/.agents/skills/bin/gstack-config get founder_resources 2>/dev/null || echo "true"
```
...
`~/.agents/skills/bin/gstack-config set founder_resources false`, then
...
```bash
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null || true)"
~/.agents/skills/bin/gstack-developer-profile --log-session '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' 2>/dev/null || true
```
...
3. Use ask_question to offer opening the resources:
...
**Otherwise**, offer via ask_question (D<N> format from the preamble):
...
```bash
~/.agents/skills/bin/gstack-telemetry-log --event-type handoff --skill office-hours --outcome accepted --session-id "$_SESSION_ID" 2>/dev/null || true
```
On D, log declined and stop:
```bash
~/.agents/skills/bin/gstack-telemetry-log --event-type handoff --skill office-hours --outcome declined --session-id "$_SESSION_ID" 2>/dev/null || true
```
