...
name: office-hours
description: >-
  YC Office Hours — two modes: Startup mode (6 forcing questions on demand reality, status quo, desperate specificity, narrowest wedge, observation, future-fit) and Builder mode (design thinking brainstorming). Produces a design doc. Use when asked to "brainstorm this", "help me think through this", "office hours", or "is this worth building".
---
...
eval "$(~/.agents/skills/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
{
  ...
  printf '\n### %s\n\n' "salience"
  ~/.agents/skills/bin/gstack-brain-cache get salience --project "$SLUG" 2>/dev/null || printf '_(no salience digest available yet)_\n'
...
> **STOP.** Before running the startup-mode diagnostic (Phase 2A: operating principles, pushback patterns, and the six forcing questions), Read `~/.agents/skills/office-hours/sections/phase-2a-startup-diagnostic.md` and execute it
...
> **STOP.** Before running the builder-mode brainstorm (Phase 2B: operating principles, the wild exemplar, and the generative questions), Read `~/.agents/skills/office-hours/sections/phase-2b-builder-brainstorm.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
- Ask via ask_question: "Should we build on this prior design or start fresh?"
...
**Privacy gate:** Before searching, use ask_question: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Use ask_question to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Use ask_question (regardless of codex availability):
...
6. **Premise revision check:** If Codex challenged an agreed premise, use ask_question:
...
Emit ONE ask_question that lists every alternative (A/B and optionally C) as numbered options, using the preamble's ask_question Format section. The ask_question call is a tool_use, not prose — write the question text and call the tool.
...
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.agents/skills/design/dist/design" ] && D="$_ROOT/.agents/skills/design/dist/design"
[ -z "$D" ] && D="$HOME/.agents/skills/design/dist/design"
...
If `$D serve` is not available or fails, fall back to ask_question:
...
If Codex is available, use ask_question:
...
```bash
~/.agents/skills/bin/gstack-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
> **STOP.** Before writing the design doc and running the tiered relationship handoff (Phases 5-6, after the conversation and alternatives are done), Read `~/.agents/skills/office-hours/sections/design-and-handoff.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
~/.agents/skills/bin/gstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one ask_question.
