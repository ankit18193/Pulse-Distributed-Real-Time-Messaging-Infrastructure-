...
description: >-
  Diataxis documentation generator: Writes clear documentation mapped to the 4 Diataxis modes (Tutorial, How-To, Reference, Explanation) directly from code. Use when asked to "generate docs", "write tutorials", or "create api reference".
---
...
2. Use ask_question to confirm scope and ask about documentation target:
...
If the plan has more than 5 documents to create, use ask_question to confirm before proceeding.
...
git diff --cached --no-color | grep '^+' | sed 's/^+//' | \
  ~/.agents/skills/bin/gstack-redact --repo-visibility "${REDACT_VIS:-unknown}" --json
