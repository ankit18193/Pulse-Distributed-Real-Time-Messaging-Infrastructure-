...
For each risky or questionable update identified in Step 2, use ask_question with:
...
3. Never regenerate a CHANGELOG entry from scratch. The entry was written by `/ship` from the
   ...
   rewriting history.
4. If an entry looks wrong or incomplete, use ask_question — do NOT silently fix it.
...
- Auto-fix minor voice adjustments. Use ask_question if a rewrite would alter meaning.
...
5. Flag any contradictions between documents. Auto-fix clear factual inconsistencies (e.g., a
   version mismatch). Use ask_question for narrative contradictions.
...
2. **Items needing description updates:** If a TODO references files or components that were
   significantly changed, its description may be stale. Use ask_question to confirm whether
   the TODO should be updated, completed, or left as-is.

3. **New deferred work:** Check the diff for `TODO`, `FIXME`, `HACK`, and `XXX` comments. For
   each one that represents meaningful deferred work (not a trivial inline note), use
   ask_question to ask whether it should be captured in TODOS.md.
...
3. **If VERSION was NOT bumped:** Use ask_question:
...
4. **If VERSION was already bumped:** Do NOT skip silently. Instead, check whether the bump
   ...
   d. **If there are significant uncovered changes:** Use ask_question explaining what the
...
```bash
~/.agents/skills/bin/gstack-issue-guard --stdin --source pr-body < /tmp/gstack-pr-body-$$.md
```
...
~/.agents/skills/bin/gstack-redact --from-file /tmp/gstack-pr-body-$$.md --repo-visibility "${REDACT_VIS:-unknown}" --json
...
```bash
NEW_TITLE=$(~/.agents/skills/bin/gstack-pr-title-rewrite.sh "$V" "$CURRENT_TITLE")
```
...
present the findings, then use ask_question ONCE:
...
**Persist the result:**
```bash
~/.agents/skills/bin/gstack-review-log '{"skill":"codex-doc-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
