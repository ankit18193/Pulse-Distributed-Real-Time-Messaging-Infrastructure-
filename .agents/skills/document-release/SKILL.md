...
description: >-
  Post-ship documentation synchronization: Cross-references recent diffs and updates README, ARCHITECTURE, AGENTS.md, and CHANGELOG to match shipped code. Use when asked to "update docs after ship", "document release", or "sync docs".
---
...
prefixing the `Antigravity session
...
the ask_question Format spawned rule: auto-choose the RECOMMENDED option, record the decision
in your completion report, and continue — never call ask_question, never render a prose
...
- Bump VERSION without asking — always use ask_question for version changes
...
> **STOP.** Before auditing each doc file and applying updates, polishing CHANGELOG voice, checking cross-doc consistency, cleaning up TODOS, the VERSION bump, and committing (Steps 2-9, after the coverage map in Step 1.5), Read `~/.agents/skills/document-release/sections/release-body.md` and execute it
